...
1. **Семантичний пошук vs BM25**
  ...
  У нашій видачі лексичний BM25 виявився ефективнішим для точних сутностей("Yann LeCun"), де важливий жорсткий збіг конкретних ключових слів. Векторний пошук беззаперечно переміг у запиті "making computers understand human emotions from text", де не було специфічних термінів, а лише загальна концепція. Загальне правило для production: BM25 варто використовувати для пошуку за ідентифікаторами, абревіатурами чи іменами, тоді як семантичний пошук ідеально підходить для запитів природною мовою, пошуку за синонімами та непрямим змістом.

2. **Вплив розміру чанка**

  Якщо чанк занадто маленький (10–15 слів), він втрачає глобальний контекст. Якщо чанк занадто великий (500+ слів), виникає ефект *"розмиття"* — головна думка губиться серед другорядних деталей. Універсального розміру не існує, він залежить від домену та вікна контексту LLM (зазвичай це 256–512 токенів), але найкращою практикою можна вважати `semantic chunking`, який спирається на логічні межі речень чи абзаців, зберігаючи цілісність думки.
  
3. **Невідповідна метрика**

  Якщо індекс налаштовано на L2, а вектори нормалізовані => результати ранжування будуть абсолютно ідентичними до видачі за Cosine Similarity. 
  
  Математично квадрат Евклідової відстані між двома векторами $a$ та $b$ дорівнює: 
  
  $L2^2 = ||a - b||^2 = ||a||^2 + ||b||^2 - 2(a \cdot b)$.
  
  Оскільки $||A|| = 1$ і $||B|| = 1$, формула спрощується до:

  $L2^2 = 2 - 2\cos(\theta)$
  
  Звідси мінімізація відстані L2 прямо відповідає максимізації косинусної схожості, тому зміниться лише абсолютне значення скорів, але не порядок релевантних документів.

4. **Обмеження Pinecone Starter**

  Головне обмеження безкоштовного тарифу Pinecone — це жорсткий ліміт у 100 000 векторів та доступ лише до одного індексу без підтримки ізольованих просторів (namespaces). Для масштабування до 10 мільйонів статей (що після чанкінгу перетвориться на десятки мільйонів векторів), я б перейшла на платний Serverless тариф або розгорнула self-hosted рішення (наприклад, Qdrant) для економії бюджету. Крім того, важкі текстові метадані (повні тексти статей) були б винесені у дешевше сховище, залишивши у векторній базі виключно ID та найнеобхідніші фільтри для прискорення пошуку.